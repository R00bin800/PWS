...
#df = pd.read_excel(r'C:\Users\user\OneDrive - Landstede Groep\school\PWS\graph_data.xlsx')
...
#finding node with lowest cost, and not checked yet
def cheapest_node_finder(weights, not_checked):
    lowest_weight = inf
    cheapest_node = ""
    for node in weights:
        if node in not_checked and weights[node] < lowest_weight:
            lowest_weight = weights[node]
            cheapest_node = node
...
if __name__ == "__main__":
    not_checked = [node for node in weights]
    node = cheapest_node_finder(weights, not_checked)
    while not_checked:
        #print(f"not checked: {not_checked}")
        weight = weights[node]
        child_weight = graph[node]
        for child in child_weight:
            if weights[child] > weight + child_weight[child]: #checking if route with new parent is shorter than current route
                weights[child] = weight + child_weight[child] #if shortest, set new weight
                parent[child] = node                          #setting new parent as parent
        not_checked.pop(not_checked.index(node))              #remove parent node from list not_checked
        node = cheapest_node_finder(weights, not_checked)     #find the next cheapest node
...
print(f"De afstand van {start} naar {stop} is {weights[stop]}")  #printing the cost to go from a to b
...
#print(time.time()- start_time)                                                                    #show the plot